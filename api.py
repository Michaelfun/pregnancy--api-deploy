...
# Ensure unpickling works even if models were saved with RuleBasedModel in __main__
sys.modules.setdefault("__main__", sys.modules.get("__main__", sys.modules[__name__]))
setattr(sys.modules["__main__"], "RuleBasedModel", RuleBasedModel)
...
# Cache classifier classes once (saves work per request)
try:
...
def _parse_bool(value, default=True):
    """
    Parse boolean-ish query params like: true/false/1/0/yes/no.
    """
    if value is None:
        return default
...
def predict():
    ...
    try:
        ...
        if details:
            # Add analysis of each vital sign (slower path)
            analysis = {}
